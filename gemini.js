...
// NEW FUNCTION: Asks Gemini to generate a realistic role-specific interview question
async function generateInterviewQuestion(role, difficulty) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`;
    
    const prompt = `
    You are an expert technical recruiter. Generate ONE highly realistic, common, or famous interview question for the following role and difficulty level.
    The question should draw from actual historical interviews at top companies to genuinely help the candidate prepare.
    ...
    The JSON structure must match this EXACTLY:
    {
        ...
        "question": "The actual text of the interview question goes here."
    }
    `;
    ...
    try {
        ...
        return { category: "General", question: "Tell me about yourself and your background." };
    }
}

// 3. Cleanly override Claude's interview engine functions
window.startInterview = async function() {
    ...
    const finalDiff = selectedDiff || "medium";
    ...
    // Show a loading text while Gemini generates the real question
    document.getElementById('qText').innerText = "Generating a tailored interview question from real-world datasets...";
    document.getElementById('qCategory').innerText = "Loading...";
    ...
    // Inject the real question into the screen layout
...
window.nextQuestion = function() {
    // Refresh the interview with a completely new question!
